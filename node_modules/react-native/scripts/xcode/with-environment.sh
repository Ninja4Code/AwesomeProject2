...
echo "Getting the node versions"
if which node ; then
  echo "Great, node is installed"
else
  echo "bad, node is nowhere to be found"
fi
NODE_BINARY=$(command -v node)
export NODE_BINARY
echo "Trying to finnd node"Y
echo $NODE_BINARY
# Override the default with the global environment
ENV_PATH="$PODS_ROOT/../.xcode.env"
echo "Finding PODS_ROOT"
echo $PODS_ROOT
echo "Finding ENV_PATH"
echo $ENV_PATH
if [ -f "$ENV_PATH" ]; then
    source "$ENV_PATH"
fi
